 ...
 */
bool is_point_inside(Point point, const Point *polygon, int n) {
    int intersections = 0;
 ...
 * @brief iou for obb
 */
float iou_obb(detectedObject *a, detectedObject *b) {
    ...
    for (int i = 0; i <= grid_x; i++) {
        for (int j = 0; j <= grid_y; j++) {
            Point test_point;
            test_point.x = small_corners[0].x + (i + 0.5) * step_x * cos(small_angle) - (j + 0.5) * step_y * sin(small_angle);
            test_point.y = small_corners[0].y + (i + 0.5) * step_x * sin(small_angle) + (j + 0.5) * step_y * cos(small_angle);

            total_count++;

            if (is_point_inside(test_point, corners_a, 4) && is_point_inside(test_point, corners_b, 4)) {
                overlap_count++;
            }
        }
    }
